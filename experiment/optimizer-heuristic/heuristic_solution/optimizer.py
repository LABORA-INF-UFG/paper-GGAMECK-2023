...
def generate_input():
    ...
    result = prom.get_current_metric_value(metric_name='sum by (kubernetes_node) (rate(node_cpu_seconds_total{mode="idle"}[5m]))')
    ...
    for from_node in crs_id_list:
        result = prom.get_current_metric_value(metric_name='sum by (job) (avg_over_time(probe_http_duration_seconds{app_kubernetes_io_instance="blackbox-node'+str(from_node)+'", phase!="resolve", phase!="tls"}[5m]))')
...
#CPU Usage by POD:
#PromQL: 
#avg by (pod) (rate(container_cpu_usage_seconds_total{pod=~".*ricplt.*",id!~".*cri-containerd.*"}[5m]))
#Python prometheus_api_client
#result = prom.get_current_metric_value(metric_name='avg by (pod) (rate(container_cpu_usage_seconds_total{pod=~".*ricplt.*",id!~".*cri-containerd.*"}[5m]))')
#pprint.pprint(result)
#[{'metric': {'pod': 'deployment-ricplt-a1mediator-6db879dc8b-x8bqd'},
