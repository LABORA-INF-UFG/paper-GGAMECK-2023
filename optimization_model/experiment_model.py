...
def run_model(q_CRs, q_RUs):
    ...
    for du in dus:
        ...
        for xApp in range(1, N_XAPPS):
            mdl.add_constraint(mdl.sum(mdl.x[it] * (links['({}, {})'.format(dus[du].CR, it[2])].delay + links['({}, {})'.format(it[3], it[4])].delay) for it in i) +
                                   ...
                                   mdl.sum(mdl.y[(du, xApp, c1)] * SDL_ACCESS[str(xApp)] * mdl.sum(mdl.u_SDL_DU[(c2, du)] * links['({}, {})'.format(c2, c1)].delay for c2 in crs) for c1 in crs) <= 10)

        # for it1 in i:
        #     for it2 in i_xApps:
        #         if it1[1] == it2[0] == du:
        #             mdl.add_constraint((mdl.x[it1] * links['({}, {})'.format(dus[it1[1]].CR, it1[2])].delay) +
        ...
        #                                         for xApp_n in TOTAL_XAPPS_CHAIN[str(it2[1])])) +
        #                                (mdl.x[it1] * links['({}, {})'.format(it1[3], it1[4])].delay) <= 5)
    ...
    # heuristic_solution_file = open("../solutions/heuristic_{}_CRs_{}_RUs.json".format(q_CRs, q_RUs))
    # json_heuristic = json.load(heuristic_solution_file)
    # json_DUs = json_heuristic["DUs"]
    # for du in json_DUs:
    #     heuristic_i.append((du["RIC"], du["DU"], du["E2T"], du["SDL"], du["DB"]))
    #     for xApp in range(1, N_XAPPS + 1):
    #         heuristic_i_xApps.append((du["DU"], xApp, du["xApp{}".format(xApp)]))
    #
    # warm_start = mdl.new_solution()
    # for i in heuristic_i:
    #     warm_start.add_var_value(mdl.x[i], 1)
    #
    # for i in heuristic_i_xApps:
    #     warm_start.add_var_value(mdl.y[i], 1)
    #
        ...
        if mdl.x[it].solution_value > 0.8:
            ...
            for it2 in i_xApps:
                if mdl.y[it2].solution_value > 0.8 and it[1] == it2[0]:
                    print("DU {} | xAPP {} | CR {}".format(it[0], it[1], it[2]))
                    DUs_RIC["xApp{}".format(it2[1])] = it2[2]
            all_DUs[str(it[1])] = DUs_RIC
